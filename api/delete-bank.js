...
export default async function handler(req, res) {
  ...
  try {
    console.log(`🗑️ Delete bank request: uid=${uid}, bankId=${bankId}`);
    ...
    if (typeof bankId === 'string' && bankId.startsWith('ins_')) {
      ...
      if (!snap.empty) {
        ...
        return res.status(200).json({ message: 'Bank deleted', deletedCount });
      }
    }
    ...
    console.error('🔥 Error deleting bank:', error?.stack || error?.message || error);
